...
const inactivityDuration = 5 * 60 * 1000; // 5 minutes of inactivity before locking

function resetInactivityTimer() {
  clearTimeout(inactivityTimeout);
  inactivityTimeout = setTimeout(lockBrowser, inactivityDuration);
}
...
function lockBrowser() {
  chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
    chrome.tabs.executeScript(
      tabs[0].id,
      { code: 'document.body.innerHTML = "<h1>Browser Locked</h1>";' }
    );
  });
}
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  if (request.type === 'password') {
    ...
    // Do something with the password, like store it or validate it
  }
});
